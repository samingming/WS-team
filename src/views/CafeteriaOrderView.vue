...
const currentMenuSlug = computed(() => route.params.menu as string)
const currentMenu = computed(() => currentCounter.value?.menus.find((menu) => menu.slug === currentMenuSlug.value))
...
<template>
  <section class="order-page">
    ...
    <template v-if="currentCounter && currentMenu">
      <div class="order-hero">
        <span class="order-icon">{{ currentCounter.icon }}</span>
        <div>
          <p class="eyebrow">{{ currentCounter.title }}</p>
          <h1>{{ currentMenu.name }}</h1>
          <p class="subtitle">{{ currentMenu.description }}</p>
        </div>
      </div>
      ...
      <div class="quantity-card">
        ...
        <div class="actions">
          <button class="qty-btn" aria-label="수량 감소" @click="changeQuantity(-1)">−</button>
          <button class="qty-btn" aria-label="수량 증가" @click="changeQuantity(1)">+</button>
        </div>
      </div>
